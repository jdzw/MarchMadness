...
team_metrics_by_season <- function(seasonletter) {
    ...
    big_wins_by_team$Freq,big_loss_by_team$Freq,ratings_season_final[,-(1:2)])
...
train_frame_model <- function(seasonletter) {
    ...
    headersB <- colnames(ratings[,-(1:3)])
    for(i in 1:length(headersB)) {
      headersB[i] <- paste("B",headersB[i],sep="_")
    }
    colnames(teamMetrics_away) <- c("TEAMID", "B_TWPCT","B_WST6", "B_SEED", "B_AWAYPCT",
                                    "B_CloseWin","B_CloseLoss","B_BigWin","B_BigLoss",headersB)
